...
exports.saveArticleType = function (req, res, next) {
    console.log('enter saveArticleType param')
...
exports.saveArticleDetail = function (req, res, next) {
    //find
    articleDetail.find({_id: /*mongoose.Types.ObjectId(*/req.body.id/*)*/}, function(err, row){
        if(err) return next(err);
        if(row.length){
            var condition = {_id : /*mongoose.Types.ObjectId(*/req.body.id/*)*/ },
                updateobj = { $set:{title:  req.body.txtTitle || '',summary:req.body.txtSummary, content:req.body.txtContent, 'fda':'fdsafds' }},
                options = {upsert : true};
            //update
            articleDetail.update(condition, updateobj, options, function(error, row){
...
exports.initManager=function (req,res,next){
    ...
    if(req.params.articleId){
        articleDetail.find({_id:req.params.articleId}, function (err, row) {
            ...
            var model = row[0]['_doc']
            res.render('./article/articleManager.html',{article:model});
        });
    }else{
        var model = articleDetailEntity
        res.render('./article/articleManager.html',{article:articleDetailEntity});
    }
};
exports.delete = function(req, res, next){
    console.log('get articleId:',  req.params["articleId"] )
    articleDetail.remove({ _id :  req.params["articleId"] }, function(err, result){
